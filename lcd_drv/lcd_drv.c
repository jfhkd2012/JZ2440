...
{
	chan &= 0xffff;
	chan >>= 16 - bf->length;		// 取16位颜色值的最高bf->length 个长度位数
	return chan << 16 - bf->offset;  // 将上面获取的颜色值放到对应的RGB 16 颜色数据中
}
...
static int lcd_drv_init(void)
{
	...
	*gpccon = 0xAAAAAAAA;	//gpio 管脚用于VD[7:0], VM, VFRAME, VLINE, VCLK, LEND
	*gpdcon = 0xAAAAAAAA; //gpio管脚用于VD[23:8]

	*gpbcon &= ~(3); //gpb0脚设置为输出引脚
	*gpbcon |= 1;
	*gpbdat &= ~1;    //输出低电平
	...
	*gpgcon |= (3 << 8);  //gpg4用作LCD_PWREN
	...
	// 3.3、分配framebuffer，并把地址告诉LCD控制器
